export async function POST(request: Request) {
  try {
    ...
    const edgeFunctionUrl = `${supabaseUrl}/functions/v1/generate-topic-breakdown`;
    ...
    if (!response.ok) {
      ...
      if (errorText.includes('<!DOCTYPE') || errorText.includes('<html>')) {
        return Response.json(
          { 
            success: false, 
            error: 'Edge function not found. Please ensure the generate-topic-breakdown function is deployed to your Supabase project.' 
          },
          { status: 500 }
        );
      }
    ...
    if (!data.success) {
      return Response.json(
        { 
          success: false, 
          error: data.error || 'Topic breakdown generation failed' 
        },
        { status: 400 }
      );
    }
    ...
    console.error('Topic breakdown generation error:', error);
    return Response.json(
      { success: false, error: error.message || 'Failed to generate topic breakdown' },
      { status: 500 }
    );
  }
}