/*
  ...
  2. Security
    - Enable RLS on user_progress_sessions
    - current_affairs_daily is public read (no RLS needed)
    - Add policies for user progress sessions
...
DO $$ 
BEGIN
    -- Drop policies for user_progress_sessions if they exist
...
) AS new_data(date, sequence_order, topic, question, answer, options, type, category, difficulty, exam_relevance, keywords, explanation, source, is_active)
WHERE NOT EXISTS (
  SELECT 1 FROM current_affairs_daily 
  WHERE date = CURRENT_DATE AND sequence_order = new_data.sequence_order
);