/*
  ...
  1. New Tables
    - `current_affairs_daily`
      ...
      - `type` (varchar(20), not null - mcq/true-false/reveal)
      - `category` (varchar(20), not null - politics/economy/international/schemes/general)
      - `difficulty` (varchar(10), not null - easy/medium/hard)
      - `exam_relevance` (integer, nullable - score 1-100)
      - `keywords` (jsonb, nullable - array of keywords)
    ...
    - `user_progress_sessions`
      ...
      - `questions_answered` (integer, nullable)
      - `correct_answers` (integer, nullable)
      - `accuracy_percentage` (double precision, nullable)
      - `xp_earned` (integer, nullable)
      - `time_spent_seconds` (integer, nullable)
      - `perfect_score` (boolean, nullable)
      - `hearts_remaining` (integer, nullable)
      - `session_completed` (boolean, nullable)
      - `created_at` (timestamp, default now())

  2. Security
    - Enable RLS on `user_progress_sessions` table
    - Add policies for users to manage their own progress sessions
    - `current_affairs_daily` is public read (no RLS needed)

  3. Performance
    - Add indexes for efficient querying by date and user
    - Add indexes for filtering active questions

  4. Sample Data
    - Insert sample current affairs questions for today
    - Questions cover various categories and types
*/
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Users can view their own progress sessions" ON user_progress_sessions;
  DROP POLICY IF EXISTS "Users can insert their own progress sessions" ON user_progress_sessions;
  DROP POLICY IF EXISTS "Users can update their own progress sessions" ON user_progress_sessions;
EXCEPTION
...
-- Insert sample current affairs questions for today (only if they don't exist)
...
WHERE NOT EXISTS (
  SELECT 1 FROM current_affairs_daily 
  WHERE date = CURRENT_DATE AND sequence_order = 1
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM current_affairs_daily 
  WHERE date = CURRENT_DATE AND sequence_order = 2
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM current_affairs_daily 
  WHERE date = CURRENT_DATE AND sequence_order = 3
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM current_affairs_daily 
  WHERE date = CURRENT_DATE AND sequence_order = 4
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM current_affairs_daily 
  WHERE date = CURRENT_DATE AND sequence_order = 5
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM current_affairs_daily 
  WHERE date = CURRENT_DATE AND sequence_order = 6
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM current_affairs_daily 
  WHERE date = CURRENT_DATE AND sequence_order = 7
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM current_affairs_daily 
  WHERE date = CURRENT_DATE AND sequence_order = 8
);